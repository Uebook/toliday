...
export class AdminService {
       ...
       async processSettlements() {
              ...
              const getRate = (key: string) => parseFloat(settings.find(s => s.key === key)?.value || '0') / 100;
              ...
              const busBookings = await this.busBookingRepository.createQueryBuilder('bb')
       ...
       private async createLedgerEntry(vendorId: string, vertical: VerticalType, amount: number, description: string, referenceId: string) {
       ...
       async getDashboardStats() {
              ...
              const hotelRevenue = await this.bookingRepository.createQueryBuilder('b').where('b.hotelId IS NOT NULL').select('SUM(b.totalAmount)', 'sum').getRawOne();
              const packageRevenue = await this.bookingRepository.createQueryBuilder('b').where('b.tourPartnerId IS NOT NULL').select('SUM(b.totalAmount)', 'sum').getRawOne();
              const busRevenue = await this.busBookingRepository.createQueryBuilder('b').select('SUM(b.totalFare)', 'sum').getRawOne();
              const cabRevenue = await this.cabBookingRepository.createQueryBuilder('b').select('SUM(b.totalAmount)', 'sum').getRawOne();
              ...
              return {
                      counts: { hotels: totalHotels, packages: totalTourPartners, buses: totalBusVendors, cabs: totalCabVendors },
                      revenue: { hotels: hRev, packages: pRev, buses: bRev, cabs: cRev, total: hRev + pRev + bRev + cRev },
       ...
       async findHotelById(id: string) {
              ...
              const offers = await this.promotionRepository.find({ where: { vendorId: id } });
              return { ...hotel, offers };
       }
       ...
       async addTourPackage(partnerId: string, data: any) {
              const pkg = this.tourPackageRepository.create({ ...data, partnerId, status: PackageStatus.ACTIVE });
       ...
       async findBusVendorById(id: string) {
              ...
              const bookings = await this.busBookingRepository.createQueryBuilder('bb')
                     .leftJoinAndSelect('bb.schedule', 's')
                     .leftJoinAndSelect('s.bus', 'b')
       ...
       async deleteCabVehicle(id: string) {
        ...
        async findAllBookings() {
               const hotelBookings = await this.bookingRepository.find({ where: { tourPartnerId: IsNull() }, relations: ['hotel'] });
               const packageBookings = await this.bookingRepository.find({ where: { hotelId: IsNull() }, relations: [] });
               const busBookings = await this.busBookingRepository.find({ relations: ['schedule', 'schedule.bus'] });
               const cabBookings = await this.cabBookingRepository.find({ relations: ['vendor', 'vehicle'] });

               const unified = [
                      ...hotelBookings.map(b => ({ ...b, vertical: 'HOTEL', amount: b.totalAmount, guestName: b.guestName })),
                      ...packageBookings.map(b => ({ ...b, vertical: 'PACKAGE', amount: b.totalAmount, guestName: b.guestName })),
                      ...busBookings.map(b => ({ ...b, vertical: 'BUS', amount: b.totalFare, guestName: b.passengerDetails?.[0]?.name })),
                      ...cabBookings.map(b => ({ ...b, vertical: 'CAB', amount: b.totalAmount, guestName: b.customerName })),
               ];
               return unified.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        }

       async findAllUsers() {
              return this.staffRepository.find({ relations: ['hotel', 'tourPartner', 'busVendor', 'cabVendor'], order: { createdAt: 'DESC' } });
       }
}
