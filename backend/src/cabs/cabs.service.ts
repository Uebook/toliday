...
export class CabsService {
    ...
    async findVehicles(vendorId: string) {
        return this.vehicleRepo.find({ where: { vendorId }, order: { createdAt: 'DESC' } });
    }
    ...
    async findDrivers(vendorId: string) {
        return this.driverRepo.find({ where: { vendorId }, order: { createdAt: 'DESC' } });
    }
    ...
    async findPricing(vendorId: string) {
        return this.pricingRepo.find({ where: { vendorId }, order: { createdAt: 'DESC' } });
    }
    ...
    async updateBookingStatus(id: string, vendorId: string, status: CabBookingStatus) {
    ...
    async getStats(vendorId: string) {
        ...
        const revenueResult = await this.bookingRepo
            ...
            .andWhere('booking.status = :status', { status: CabBookingStatus.COMPLETED })
            .getRawOne();
