...
export class InventoryService {
  ...
  async bulkUpdate(hotelId: string, updateDto: UpdateInventoryDto) {
    const { roomTypeId, startDate, endDate, totalRooms, priceOverride } =
      updateDto;
  ...
  async getInventory(hotelId: string, startDate: string, endDate: string) {
    ...
    for (let dt = new Date(start); dt < end; dt.setDate(dt.getDate() + 1)) {
      ...
      if (!inv || inv.availableRooms < count) {
        throw new BadRequestException(
          `Rooms not available for date: ${dateStr}`,
        );
      }
