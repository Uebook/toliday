...
export class PackagesService {
       ...
       async findOnePackage(id: string, partnerId: string) {
              const pkg = await this.tourPackageRepository.findOne({ where: { id, partnerId } });
       ...
       async getStatsSummary(partnerId: string) {
              const totalPackages = await this.tourPackageRepository.count({ where: { partnerId } });
              const totalTours = await this.tourRepository.count({ where: { partnerId } });
       ...
       async getPaymentTransactions(partnerId: string) {
              ...
              return bookings.map(b => ({
                     ...
                     status: b.status === BookingStatus.COMPLETED ? 'PAID' : (b.status === BookingStatus.CANCELLED ? 'REFUNDED' : 'PENDING'),
        ...
        async getTiers(packageId: string) {
            return this.tierRepository.find({ where: { packageId }, order: { paxMin: 'ASC' } });
        }
        ...
        async getDepartures(packageId: string) {
            return this.departureRepository.find({ where: { packageId }, order: { date: 'ASC' } });
        }
