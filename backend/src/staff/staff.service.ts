...
export class StaffService {
       ...
       async findAll(hotelId?: string, tourPartnerId?: string, busVendorId?: string, cabVendorId?: string) {
              const where: any = {};
       ...
       async create(partnerId: string, type: 'hotel' | 'tour' | 'bus' | 'cab', dto: {
              name: string; email: string; password: string;
              role?: string; phone?: string; permissions?: Record<string, boolean>;
       }) {
              try {
                     console.log('[DEBUG] Staff Creation Request:', { partnerId, type, email: dto.email, role: dto.role });
                     
                     if (!partnerId) {
                            console.error('[ERROR] PartnerId is missing in staff creation');
                            throw new BadRequestException('Partner ID is missing. Please re-login.');
                     }
       ...
       async update(id: string, dto: Partial<{
              ...
              role: string; isActive: boolean; permissions: Record<string, boolean>;
              twoFactorEnabled: boolean; emailNotifications: boolean; smsNotifications: boolean;
       }>) {
