...
export class StatsService {
       ...
       async getSummary(hotelId: string) {
              ...
              const totalRevenue = allBookings
                     .filter(b => [BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN, BookingStatus.CHECKED_OUT].includes(b.status))
                     .reduce((sum, b) => sum + Number(b.totalAmount), 0);

              const checkInsToday = allBookings.filter(b => b.startDate === todayStr && b.status === BookingStatus.CONFIRMED).length;
              const checkOutsToday = allBookings.filter(b => b.endDate === todayStr && b.status === BookingStatus.CHECKED_IN).length;
              ...
              const activeStay = allBookings.filter(b =>
       ...
       async getReports(hotelId: string, period: '7d' | '30d' | '90d' = '7d') {
              ...
              const dailyData: {
                     ...
                     cancellations: number; occupancy: number;
              }[] = [];
              ...
              for (let i = days - 1; i >= 0; i--) {
                     ...
                     const dayBookings = allBookings.filter(b => b.startDate === dayStr);
                     const revenue = dayBookings
                            .filter(b => [BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN, BookingStatus.CHECKED_OUT].includes(b.status))
                            .reduce((s, b) => s + Number(b.totalAmount), 0);
                     const bookingsCount = dayBookings.filter(b => b.status !== BookingStatus.CANCELLED).length;
                     const cancellationsCount = dayBookings.filter(b => b.status === BookingStatus.CANCELLED).length;
                     const activeDay = allBookings.filter(b =>
                            b.startDate <= dayStr && b.endDate > dayStr &&
                            [BookingStatus.CONFIRMED, BookingStatus.CHECKED_IN].includes(b.status)
                     ).length;
                     const occupancy = Math.min(100, Math.round((activeDay / 10) * 100)); // assumes ~10 rooms total

                     dailyData.push({ date: dateLabel, revenue, bookings: bookingsCount, cancellations: cancellationsCount, occupancy });
              }
              ...
              return { dailyData, totalRevenue, totalBookings, totalCancellations, avgOccupancy, sourceData };
       }
}
