...
export default function BusesFinancePage() {
       const [search, setSearch] = useState('');

       
       const { data: transactions = [], isLoading, error } = useQuery({
              queryKey: ['ledger', '/finance/admin/ledger?vertical=BUS'],
              queryFn: async () => {
                     const res = await api.get('/finance/admin/ledger?vertical=BUS');
       ...
       const filteredTransactions = transactions.filter((t: any) => 
              ...
              t.buse.toLowerCase().includes(search.toLowerCase())
       );
       ...
       const columns = [
              ...
              },
              {
                     ...
                     accessor: 'hotel',
                     render: (trx: any) => (
                            <div className="font-bold text-slate-700 text-sm">{trx.buse}</div>
                     )
              },
              {
                     header: 'Direction',
              ...
              },
              {
                     ...
                     render: (trx: any) => (
                            <div>
                                   ...
                                   {trx.type === 'INCOME' && (
                                          <div className="text-[10px] font-black text-amber-500 uppercase tracking-widest mt-1">
       ...
       const headerAction = (
              <button className="px-6 py-3 bg-amber-600 hover:bg-amber-500 text-foreground rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transition-all shadow-xl shadow-amber-600/20 active:scale-95 group">
       ...
       return (
              <div className="p-6 md:p-8 space-y-6 md:space-y-8 animate-fadeIn max-w-[1600px] mx-auto min-h-full">
                     <Topbar title="Finance & Payments" subtitle="Manage revenue, commissions, and payouts for bus partners" />
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div className="ios-platter p-6 rounded-[24px] border border-border/10 shadow-[0_4px_20px_rgba(0,0,0,0.01)] transition-transform hover:scale-[1.03]">
                                   <div className="p-8 relative z-10">
                                          <div className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2">Platform Commissions</div>
