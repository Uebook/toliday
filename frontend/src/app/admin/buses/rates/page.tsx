       ...
       return (
              <div className="p-6 md:p-8 space-y-6 md:space-y-8 animate-fadeIn max-w-[1600px] mx-auto min-h-full">
                     ...
                     <div className="relative z-20">
                            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3 block">Select Bus Operator</label>
                            <div 
                            ...
                            >
                                   {activeVendor ? (
                                          ...
                                          <span className="text-sm font-bold text-muted-foreground">Select a bus partner...</span>
                                   )}
                                                 ...
                                                 {vendors.map(vendor => (
                                                        ...
                                                        >
                                                               <div>
                                                                      ...
                                                                      <div className={`text-[10px] font-bold uppercase tracking-widest mt-0.5 ${selectedVendor === vendor.id ? 'text-amber-500' : 'text-muted-foreground'}`}>{vendor.location}</div>
                                                               </div>
                     ...
                     {!selectedVendor ? (
                            <div className="rounded-[2.5rem] border-dashed border-2 border-slate-200 bg-transparent shadow-none">
                                   <div className="p-16 flex flex-col items-center justify-center text-center">
                                          ...
                                          <h3 className="text-xl font-black text-foreground mb-2">No Bus Operator Selected</h3>
                                          <p className="text-muted-foreground font-bold max-w-sm">Please select a bus partner from the dropdown above to view and manage their specific rates and availability matrix.</p>
                                   </div>
                            </div>
                     ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-fadeIn">
                                   <div className="lg:col-span-1 rounded-[28px] border-border/10 shadow-[0_12px_40px_rgba(0,0,0,0.02)]">
                                          ...
                                          <div className="p-8 pt-0 space-y-4">
                                                 ...
                                                 <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-sm font-bold text-muted-foreground cursor-pointer hover:border-amber-500 transition-colors">
                                                 ...
                                                 <div className="pt-6 border-t border-slate-100 space-y-3">
                                                        ...
                                                        <div className="flex gap-2 flex-wrap">
                                                               <span onClick={() => setFilterType('ALL')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest cursor-pointer ${filterType === 'ALL' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-muted-foreground hover:bg-slate-100'}`}>All Rooms</span>
                                                               <span onClick={() => setFilterType('LOW')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest cursor-pointer ${filterType === 'LOW' ? 'bg-amber-50 text-amber-600' : 'bg-slate-50 text-muted-foreground hover:bg-slate-100'}`}>Low Inventory</span>
                                                        </div>
                                                 </div>
                                          </div>
                                   </div>
