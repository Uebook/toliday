...
export default function CabsBookingsPage() {
       const queryClient = useQueryClient();
       const [search, setSearch] = useState('');

       const { data: bookings = [], isLoading } = useQuery({
              queryKey: ['cabs-bookings'],
              queryFn: async () => {
                     const res = await api.get('/bookings/admin/all');
                     // Note: In real app, endpoint should be filtered by vertical
                     return res.data.filter((b: any) => b.cabId);
              }
       });
       ...
       const updateStatusMutation = useMutation({
              ...
              onSuccess: () => {
                     queryClient.invalidateQueries({ queryKey: ['cabs-bookings'] });
       ...
       const columns = [
              {
                     ...
                     render: (booking: any) => (
                            <div>
                                   ...
                                   <div className="text-xs text-muted-foreground font-bold mt-1">{booking.guestEmail}</div>
                                   <div className="text-[10px] text-rose-500 font-black uppercase tracking-widest mt-1">Ref: {booking.bookingReference}</div>
                            </div>
                     )
              },
              {
                     ...
                     render: (booking: any) => (
                            <div className="flex items-center gap-3">
                                   ...
                                   <div>
                                          <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">HOTEL</div>
                                          <div className="text-xs font-bold text-slate-700 truncate max-w-[150px]">{booking.cab?.name || 'Unknown Property'}</div>
                                   </div>
                            </div>
                     )
              },
              {
                     ...
                     render: (booking: any) => (
                            <div>
                                   ...
                                   <div className="text-xs text-muted-foreground font-bold mt-1">to {new Date(booking.endDate).toLocaleDateString()}</div>
                            </div>
                     )
              },
              {
       ...
       return (
              <div className="p-6 md:p-8 space-y-6 md:space-y-8 animate-fadeIn max-w-[1600px] mx-auto min-h-full">
                     <Topbar title="Bookings & Reservations" subtitle="Manage all incoming bookings for cab partners" />
