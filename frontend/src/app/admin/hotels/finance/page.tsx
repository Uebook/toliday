...
export default function HotelsFinancePage() {
       ...
       const { data: transactions = [], isLoading, error } = useQuery({
              queryKey: ['ledger', '/finance/admin/ledger?vertical=HOTEL'],
              queryFn: async () => {
                     const res = await api.get('/finance/admin/ledger?vertical=HOTEL');
              ...
              <div className="p-8 lg:p-12 animate-fadeIn space-y-8">
                     <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div>
                                   ...
                                   <p className="text-slate-400 font-bold mt-2">Manage revenue, commissions, and payouts for hotel partners</p>
                            </div>
                     </header>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <Card className="rounded-[2rem] border-slate-100 shadow-xl overflow-hidden relative group hover:-translate-y-1 transition-transform">
                                   <CardContent className="p-8 relative z-10">
                                          <div className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-2">Platform Commissions</div>
