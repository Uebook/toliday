       ...
       return (
              <div className="p-8 lg:p-12 animate-fadeIn space-y-8">
                     <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            ...
                            {selectedVendor && (
                                   <button onClick={handleSave} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest flex items-center gap-3 transition-all shadow-xl shadow-indigo-600/20 active:scale-95">
                     ...
                     <div className="relative z-20">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Select Property Partner</label>
                            <div 
                            ...
                            >
                                   {activeVendor ? (
                                          ...
                                          <span className="text-sm font-bold text-slate-400">Select a property...</span>
                                   )}
                                                 ...
                                                 {vendors.map(vendor => (
                                                        ...
                                                        >
                                                               <div>
                                                                      ...
                                                                      <div className={`text-[10px] font-bold uppercase tracking-widest mt-0.5 ${selectedVendor === vendor.id ? 'text-indigo-500' : 'text-slate-400'}`}>{vendor.location}</div>
                                                               </div>
                     ...
                     {!selectedVendor ? (
                            <Card className="rounded-[2.5rem] border-dashed border-2 border-slate-200 bg-transparent shadow-none">
                                   <CardContent className="p-16 flex flex-col items-center justify-center text-center">
                                          ...
                                          <h3 className="text-xl font-black text-slate-900 mb-2">No Property Selected</h3>
                                          <p className="text-slate-400 font-bold max-w-sm">Please select a hotel partner from the dropdown above to view and manage their specific rates and availability matrix.</p>
                                   </CardContent>
                            </Card>
                     ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-fadeIn">
                                   <Card className="lg:col-span-1 rounded-[2.5rem] border-slate-100 shadow-xl">
                                          ...
                                          <CardContent className="p-8 pt-0 space-y-4">
                                                 ...
                                                 <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-sm font-bold text-slate-400 cursor-pointer hover:border-indigo-500 transition-colors">
                                                 ...
                                                 <div className="pt-6 border-t border-slate-100 space-y-3">
                                                        ...
                                                        <div className="flex gap-2 flex-wrap">
                                                               <span onClick={() => setFilterType('ALL')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest cursor-pointer ${filterType === 'ALL' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>All Rooms</span>
                                                               <span onClick={() => setFilterType('LOW')} className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-widest cursor-pointer ${filterType === 'LOW' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>Low Inventory</span>
                                                        </div>
                                                 </div>
                                          </CardContent>
                                   </Card>
