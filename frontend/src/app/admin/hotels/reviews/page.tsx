...
export default function HotelReviewsPage() {
       ...
       const { data: reviews = [], isLoading } = useQuery({
              queryKey: ['reviews', '/reviews/admin/all?vertical=HOTEL'],
              queryFn: async () => {
                     const res = await api.get('/reviews/admin/all?vertical=HOTEL');
                     return res.data.map((r: any) => ({
                            id: r.id,
                            hotelName: r.vendorName,
       ...
       const updateStatusMutation = useMutation({
              ...
              onSuccess: () => {
                     queryClient.invalidateQueries({ queryKey: ['reviews', '/reviews/admin/all?vertical=HOTEL'] });
