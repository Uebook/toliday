...
export default function PackagesReviewsPage() {
       ...
       const { data: reviews = [], isLoading } = useQuery({
              queryKey: ['reviews', '/reviews/admin/all?vertical=PACKAGE'],
              queryFn: async () => {
       ...
       const updateStatusMutation = useMutation({
              ...
              onSuccess: () => {
                     queryClient.invalidateQueries({ queryKey: ['reviews', '/reviews/admin/all?vertical=PACKAGE'] });
                     ...
                     <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div>
                                   <div className="flex items-center gap-3 mb-2">
                                          ...
                                          <span className="text-slate-400 font-black uppercase tracking-[0.2em] text-[10px]">Tour Operations</span>
                                   </div>
                                   <h1 className="text-4xl font-black text-slate-900 tracking-tight">Reviews & CX Moderation</h1>
                                   <p className="text-slate-400 font-bold mt-2">Monitor and moderate guest feedback across all tour partners</p>
                            </div>
                     </header>
                            ...
                            <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                   <div className="flex gap-4">
                                          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm min-w-[150px]">
                                                 ...
                                                 <div className="text-2xl font-black text-slate-900 flex items-center gap-2">
                                                        4.5 <Star className="text-amber-400 fill-amber-400" size={20} />
                                                 </div>
                                          </div>
                                          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm min-w-[150px]">
                                   ...
                                   <div className="relative">
                                          ...
                                          <input
                                                 ...
                                                 className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none w-full md:w-80 transition-all"
                                          />
                                   </div>
                            </CardHeader>
                                   ...
                                   {isLoading ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
                                          {filteredReviews.map((review: any) => (
                                                 <Card key={review.id} className="relative overflow-hidden group hover:border-emerald-200 transition-all shadow-sm hover:shadow-xl bg-white border-slate-100 rounded-[2rem]">
